...
def main(approach, id_dataset, num_classes, ood_dataset, attack):
    # model params
    batch_size = 100
    model = "resnet"
...
if __name__ == '__main__':
    ...
    adversarial_attacks = ["deepfool", "fgsm", "pgd"]
    ...
    arr_approach_names = [  # 'shine_min', 'feature_min', 'shine_avg', 'feature_avg',
        ...
        'feature_hybrid_avg', 'feature_min', 'feature_avg']
    for attack in adversarial_attacks:
        m_true = None
        for approach in arr_approach_names:
            monitor_results, m_true = main(approach, id_dataset, num_classes, ood_dataset, attack)
            # saving results
            arr_results.append(monitor_results)

        save_test_results(arr_results, arr_approach_names, id_dataset, ood_type, attack, m_true)

        ground_truth, results = load_results_test(arr_approach_names, id_dataset, ood_type, attack)

        print_results_test(arr_approach_names, results, ground_truth, attack)
