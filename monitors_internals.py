...
class MahalanobisMonitor:
    ...
    def predict(self, features, predictions):
        ...
        if len(predictions)>1:
            ...
            if self.is_tied:
                maha_squared = self._compute_mahalanobis(features, self.mean[pred], self.precision)
            else:
                maha_squared = self._compute_mahalanobis(features, self.mean[pred], self.precision[pred])
