...
'''
    ...
    if incoming_shine_pdf_tp <= np.average(pdfs_tp_source_2):
        return True
    elif incoming_shine_pdf_tp <= np.average(pdfs_fn_source_2):
        return True
    else:
        return False
'''
...
def main(model, approach, percentage_repr_tp, id_dataset, num_classes, ood_dataset, transform, magnitude):
    # model params
    batch_size = 100
    ...
    dataset_ood = Dataset(ood_dataset, "test", model, batch_size=batch_size, adversarial_attack=transform)
    ...
    # k = 250 # for cifar10 5000 instances ~ 5% of data
    features_tp_by_class = {}
    similarities_TP_by_class = {}
    features_fn_by_class = {}
    similarities_FN_by_class = {}
...
if __name__ == '__main__':
    ...
    additional_transforms = ["deepfool", "fgsm", "pgd"]
    ...
    for transform in additional_transforms:
        ...
        for approach in arr_approach_names:
            monitor_results, m_true = main(model, approach, percentage_repr_tp,
                                           id_dataset, num_classes, ood_dataset, transform, magnitude)
            # saving results
