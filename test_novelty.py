...
def calculate_feature_similarity_incoming_feature(features_fn, incoming_feature):

    scores_FN = []
    # similarities
    for fn in features_fn:
        cosine_similarity = 1 - spatial.distance.cosine(incoming_feature, fn)
        scores_FN.append(cosine_similarity)

    return scores_FN


def calculate_feature_similarity(features_fn, features_tp):
    scores_FN = []
    # similarities
    for fn in features_fn:
        for tp in features_tp:
...
def calculate_feature_similarity_thresholds(num_classes, features, y, ml_predictions):
    ...
    for c in range(num_classes):
        ...
        if len(ind_incorrect_ML) > 1:
            ...
            scores_FN = calculate_feature_similarity(features_fn, features_tp)
    ...
    return features_fn_by_class, similarities_FN_by_class
...
def predict_with_tp_tp_fn_hybrid(incoming_data1, incoming_data2,
                                 pdfs_tp_source_1, density_estimator_tp_source_1,
                                 pdfs_tp_source_2, density_estimator_tp_source_2,
                                 data_fn_source_1, similarity_threshold_fn_source_1,
                                 data_type, threshold):
    incoming_pdf = None
    new_pdf = None
    threshold_pdf_tp = None
    ...
    if "shine" in data_type:
        matrix_shine = shine.create_matrix_shine(incoming_data1)
        incoming_pdf = np.exp(density_estimator_tp_source_1.score_samples(matrix_shine))[0]
    elif "feature" in data_type:
        incoming_pdf = np.exp(density_estimator_tp_source_1.score_samples(incoming_data1))[0]
    ...
    if incoming_pdf >= threshold_pdf_tp:
        ...
        # inconclusive, let's do a check using a different source
        if "shine" in data_type:
            new_pdf = np.exp(density_estimator_tp_source_2.score_samples(incoming_data2))[0]
        elif "feature" in data_type:
            matrix_shine = shine.create_matrix_shine(incoming_data2)
            new_pdf = np.exp(density_estimator_tp_source_2.score_samples(matrix_shine))[0]

        if new_pdf <= threshold_pdf_fn:
            return True
        else:
            # let's do the last check by inspecting the similarity of incoming image to the FN images
            scores_FN = calculate_feature_similarity(data_fn_source_1, incoming_data1)
            # calculate_feature_similarity_incoming_feature(data_fn_source_1, incoming_data1)
            if np.mean(scores_FN) >= np.mean(similarity_threshold_fn_source_1):
                return False
            else:
                return True
...
def main(approach, dataset_train, num_classes, dataset_test, dataset_ood, feature_extractor):
    ...
    # calculating feature similarities between incorrect and correct predictions on the training set
    features_fn_by_class, similarities_FN_by_class = calculate_feature_similarity_thresholds(
        num_classes, features_train[id_layer_monitored], lab_train, pred_train)
    ...
    for x, pred, feature, softmax, logits, label in tqdm(
        ...
        if "hybrid" in approach:
            # hybrid approaches
            #result = predict_with_tp_hybrid(data_source_1, data_source_2, arr_pdf_tp_source_1, arr_kde_tp_source_1,
            #                       arr_pdf_tp_source_2, arr_kde_tp_source_2, approach, threshold_type)
            ...
            #result = predict_with_tp_tp_fn_hybrid(data_source_1, data_source_2, arr_pdf_tp_source_1, arr_kde_tp_source_1,
            #                                      arr_pdf_fn_source_2, arr_kde_fn_source_2,
            #                                      features_fn_by_class[pred], similarities_FN_by_class[pred],
            #                                      approach, threshold_type)
...
if __name__ == '__main__':
    ...
    arr_approach_names = [  # 'shine_min', 'feature_min', 'shine_avg', 'feature_avg',
        ...
        # 'feature_min', 'feature_avg',
        'feature_hybrid_avg']
    m_true = None
    for approach in arr_approach_names:
        monitor_results, m_true = main(approach, dataset_train, num_classes, dataset_test, dataset_ood,
                                       feature_extractor)
        # saving results
        arr_results.append(monitor_results)

    save_test_results(arr_results, arr_approach_names, id_dataset, ood_type, ood_dataset, m_true)

    ground_truth, results = load_results_test(arr_approach_names, id_dataset, ood_type, ood_dataset)

    print_results_test(arr_approach_names, results, ground_truth, ood_dataset)
