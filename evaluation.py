...
class Evaluator:
    ...
    def get_metrics(self, scores_id, scores_ood):
        ...
        if self.scores.dtype == "bool":
            ...
            thresh, f1, precision, recall = get_optimal_threshold_f1(self.scores, self.y_true)
...
def get_optimal_threshold_f1(scores, y_true):
    ...
    return thresh, f1, precision, recall
...
def plot_roc_curve(scores_test, scores_ood):
    y = np.concatenate([scores_test, scores_ood])
    y_true = np.array([1] * scores_test.shape[0] + [0] * scores_ood.shape[0])
...
def plot_precision_recall_curve(scores_test, scores_ood):
    y = np.concatenate([scores_test, scores_ood])
    y_true = np.array([1] * scores_test.shape[0] + [0] * scores_ood.shape[0])
