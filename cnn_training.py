...
def train(net, criterion, optimizer, trainloader, num_epochs, device, path, use_gpu=False):
    # self.net = net
...
id_dataset = 'fashion_mnist'  # mnist fashion_mnist
num_classes = 10
datasets_path = '/Data'
dataset = None
batch_size = 100
...
if id_dataset == 'mnist':
    dataset = torchvision.datasets.MNIST(root=datasets_path, train=True, download=True, transform=transform)

elif id_dataset == 'fashion_mnist':
    dataset = torchvision.datasets.FashionMNIST(root=datasets_path, train=True, download=True, transform=transform)

path_to_save_weights = os.path.join(os.getcwd(), 'weights', 'cnn_{}.pth'.format(id_dataset))
...
train(model, criterion, optimizer, dataloader, epochs, device, path_to_save_weights)
